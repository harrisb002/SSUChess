...
const handler = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  try {
    ...
    if (
      ...
      throw new Error(
        "THIRDWEB_ENGINE_URL, THIRDWEB_ENGINE_ACCESS_TOKEN, and THIRDWEB_ENGINE_WALLET must be set in the environment"
      );
    }
    ...
    const claimTokens = await engine.erc20.mintTo(
      // Minting tokens because it is not a drop contract
      "sepolia",
      SQUARES_CONTRACT_ADDRESS,
      THIRDWEB_ENGINE_WALLET,
      {
